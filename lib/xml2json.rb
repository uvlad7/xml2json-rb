...
module Xml2Json
  ...
  #      # @option opts [nil, "1.0", "1.1", #to_s] version (Xml2Json::Version::XML10) XML Declaration 'version' field, default if nil
  #      # @raise [RuntimeError] if +version.to_s+ returns unsupported string, for now only +"1.0"+ and +"1.1"+ are supported
  ...
  #      # @option opts [String, #to_str] indent_char (" ") Character used for indentation. This option will be ignored if +indent+ is false
  #      # @raise [TypeError] If +indent_char+ is more than one character or can not be encoded as UTF-8
  #      # @note It's not enough for the Ruby +indent_char+ string to be UTF-8 encoded, not sure if that's a bug or a feature of Magnus
  #      # @option opts [Integer, #to_int] indent_size (2) Number of characters used for indentation. This option will be ignored if +indent+ is false
  ...
  #      # @option opts [bool] ignore_attrs (false) Setting this to true will skip adding element attributes and create text nodes
  #      # @option opts [bool] merge_attrs (false) Merge all XML attributes and child elements as properties of the parent, instead of keying attributes off of the child attribute object. This option will be ignored if +ignore_attrs+ is true
  ...
  #      # @option opts [bool] explicit_array (true) Always put the child nodes in an array, otherwise an array is only created if there is more than one child
  #      # @option opts [bool] explicit_charkey (false) Always store character data under +charkey+ even if there are no other text elements stored inside the tag
